...
def Partial_Client_Selection(args, model, mode='pretrain'):
    ...
    # MOD: skip this entire checkpoint loading block when using plain ViT (paper finetune)
    if (mode=='finetune' or mode=='linprob') and args.finetune and (not str(args.model_name).startswith('vit')):
        ...
        if args.model_name == 'beit':
            ...
            # Only add fc_norm.* if the model actually has it
            if 'fc_norm.weight' in msg.missing_keys or 'fc_norm.bias' in msg.missing_keys:
                expected_missing.update({'fc_norm.weight', 'fc_norm.bias'})
            
            # Assert only expected keys are missing
            assert set(msg.missing_keys).issubset(expected_missing), f"Unexpected missing keys: {msg.missing_keys}"
    ...
    for proxy_single_client in args.proxy_clients:
        ...
        # MOD: add ViT finetune LR/batch handling (same rule as MAE branch)
        elif str(args.model_name).startswith('vit'):
            total_batch_size = args.batch_size * args.accum_iter * num_tasks
            if args.lr is None:  # only base_lr is specified
                args.lr = args.blr * total_batch_size / 256
        ...
        if mode == 'pretrain':
            ...
            # MOD: plain ViT finetune uses same LRD path as MAE finetune
            elif str(args.model_name).startswith('vit'):
                param_groups = param_groups_lrd(model_without_ddp, args.weight_decay,
                    no_weight_decay_list=model_without_ddp.no_weight_decay(),
                    layer_decay=args.layer_decay
        ...
        #we commneted this part bcz data labels were impalance 73/27 so it breaks the training finetune. then i define weight to solve
            ...
            if mixup_fn is not None:
                ...
                # ---- START: MODIFICATION FOR WEIGHTED LOSS Ariz----
                # Define weights inversely proportional to class frequencies.
                # Class 0: 73.48%, Class 1: 26.52%
                class_weights = torch.tensor([1/0.7348, 1/0.2652]).to(device) 
                criterion = torch.nn.CrossEntropyLoss(weight=class_weights)
            criterion_all[proxy_single_client] = criterion             #this line is added after weighting loss bcz  
    ...
    # MOD: add explicit ViT branch for finetune (paper-style)
    elif str(args.model_name).startswith('vit'):
        return model_all, optimizer_all, criterion_all, loss_scaler_all, mixup_fn_all
