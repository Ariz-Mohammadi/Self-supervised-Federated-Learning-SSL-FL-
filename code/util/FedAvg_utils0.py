...
def Partial_Client_Selection(args, model, mode='pretrain'):
    ...
    if (mode=='finetune' or mode=='linprob') and args.finetune:
        ...
        if mode == 'finetune':    
            trunc_normal_(model.cls_head.weight, std=2e-5)
        elif mode == 'linprob':
            trunc_normal_(model.cls_head.weight, std=0.01)
        
            # for linear prob only
            # hack: revise model's head with BN
            model.cls_head = torch.nn.Sequential(
                torch.nn.BatchNorm1d(model.cls_head.in_features, affine=False, eps=1e-6),
                model.cls_head
            )
            ...
            for _, p in model.named_parameters():
                p.requires_grad = False
            for _, p in model.cls_head.named_parameters():
                p.requires_grad = True
