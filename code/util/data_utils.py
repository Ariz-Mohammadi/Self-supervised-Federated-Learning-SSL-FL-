...
class DatasetFLFinetune(data.Dataset):
    ...
    def __init__(self, args, phase, mode='finetune'):
        ...
        if not is_train:
            args.single_client = f'{self.phase}.csv'
            if args.split_type == 'central':
                cur_clint_path = os.path.join(args.data_path, args.single_client)  # /data/Retina/test.csv
            else:
                cur_clint_path = os.path.join(args.data_path, f'{args.n_clients}_clients', args.split_type, args.single_client)  # /data/Retina/5_clients/split1/test.csv
        else:
            if args.split_type == 'central':
                cur_clint_path = os.path.join(args.data_path, args.split_type, args.single_client)  # /data/Retina/central/train.csv
            else:
                cur_clint_path = os.path.join(args.data_path, f'{args.n_clients}_clients', 
                                              args.split_type, args.single_client)  # /data/Retina/5_clients/split1/client_X.csv
    ...
    def __getitem__(self, index):
        ...
        # Detect file type and load image
        ...
        if img.ndim == 2:  # grayscale
            ...
            img = resize(img, (256, 256, min(img.shape[2], 3)))  # keep at most 3 channels
        else:
...
# Initialize accuracy dictionaries
    ...
    # Get client files
    args.dis_cvs_files = [f for f in os.listdir(os.path.join(args.data_path, f'{args.n_clients}_clients/{args.split_type}')) if f.endswith('.csv') and f.startswith('client_')]
    for single_client in args.dis_cvs_files:
        args.best_acc[single_client] = 0 if args.nb_classes > 1 else 999  # Default for best accuracy
        args.current_acc[single_client] = 0  # Default for current accuracy
