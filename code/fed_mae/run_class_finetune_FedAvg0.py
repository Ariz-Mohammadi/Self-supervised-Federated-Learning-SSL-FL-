...
class TBWriter(_TBWriter):
    def __init__(self, *args, **kwargs):
        ...
        self.step = 0
        self.writer = self  # script sometimes calls writer.add_scalar(...)
    ...
    def update(self, test_acc1=None, test_acc5=None, test_loss=None, head="perf", step=None):
        if test_acc1 is not None:
            self.add_scalar(f"{head}/test_acc1", test_acc1, step)
        if test_acc5 is not None:
            self.add_scalar(f"{head}/test_acc5", test_acc5, step)
        if test_loss is not None:
            self.add_scalar(f"{head}/test_loss", test_loss, step)


import timm
#assert timm.__version__ == "0.3.2" # version check  because Ariz upgraded the cuda to a new version then tim to newone so ...
...
#import fed_mae.models_vit as models_vit # we commented this because we used the models_mae.py like the paper
...
def get_args():
    ...
    ###we added the next parser because we modified the models_mae.py so we added a flag
    parser.add_argument('--use_class_head', action='store_true', help='Enable classification head during fine-tuning')
...
def main(args, model):
    ...
    if args.log_dir is not None:
        ...
        if log_writer is not None:
            print(f"Log Writer Type: {type(log_writer)}, Has update: {hasattr(log_writer, 'update')}")
    else:
        log_writer = None
...
# ---------- Train! (use different clients)    
    ...
    while True:
        ...
        if data_loader_val is not None:
            ...
            if log_writer is not None:
                log_writer.update(test_acc1=test_stats.get('acc1', 0), head="perf", step=epoch)
                log_writer.update(test_acc5=test_stats.get('acc5', 0), head="perf", step=epoch)
                log_writer.update(test_loss=test_stats.get('loss', 0), head="perf", step=epoch)
...
if __name__ == '__main__':
    ...
    model = models_mae.__dict__[args.model](norm_pix_loss=False,use_class_head=True,num_classes=args.nb_classes)  
    #we added the above code because we used a flag in mae which is pretrained mae we freeze the encoder part later we use it for classification.
    # the original code wrote models_vit... but if we want to go ahead according to the paper, we must use the mae.
