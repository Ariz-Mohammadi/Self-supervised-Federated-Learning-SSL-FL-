...
def get_args():
    parser = argparse.ArgumentParser(
        'Fed-MAE finetuning on COVID-FL with FedAvg / FedProx',
        add_help=False
    )
    ...
    # FedProx parameter: mu = 0 means pure FedAvg
    parser.add_argument('--fedprox_mu', type=float, default=1e-3,
                        help='FedProx proximal coefficient (0 disables FedProx)')
...
def main(args, model):
    ...
    # =====================================================
    # FedAvg / FedProx training loop
    # =====================================================
    print("========== FEDAVG / FEDPROX TRAINING LOOP ==========")
    ...
    while epoch <= args.max_communication_rounds:
        ...
        # -------------------------------------------------
        # FedProx reference weights (global at start of round)
        # -------------------------------------------------
        global_state = None
        if args.fedprox_mu > 0 and len(selected_clients) > 0:
            c0 = selected_clients[0]
            global_state = {
                k: v.detach().clone()
                for k, v in model_all[c0].state_dict().items()
            }
        ...
        for c in selected_clients:
            ...
            for _ in range(args.E_epoch):
                # Use custom loop to add FedProx term on top of train_one_epoch?
                # Easiest is to rely on train_one_epoch and then manually adjust loss,
                # but train_one_epoch already does backward. So here we reimplement
                # a simple per-batch loop if FedProx is used.
                if args.fedprox_mu <= 0:
                    # Pure FedAvg ? use standard helper
                    ...
                    # FedProx local update: manual loop
                    model_c.to(device)
                    model_c.train()
                    for samples, targets in loader_train:
                        samples = samples.to(device, non_blocking=True)
                        targets = targets.to(device, non_blocking=True)

                        if mixup_fn_c is not None:
                            samples, targets = mixup_fn_c(samples, targets)

                        outputs = model_c(samples)
                        ce_loss = criterion_c(outputs, targets)
                        ...
                        if global_state is not None:
                            for name, p in model_c.named_parameters():
                                ...
                                if name in global_state:
                                    prox_term += (p - global_state[name].to(p.device)).pow(2).sum()

                        loss = ce_loss + 0.5 * args.fedprox_mu * prox_term

                        optimizer_c.zero_grad()
                        loss.backward()
                        if args.clip_grad is not None:
                            torch.nn.utils.clip_grad_norm_(model_c.parameters(), args.clip_grad)
                        optimizer_c.step()
