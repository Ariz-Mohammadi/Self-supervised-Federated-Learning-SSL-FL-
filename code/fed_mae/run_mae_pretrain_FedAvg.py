...
# --- Shim for TB writer without set_step ---
# put near the other imports, replacing any previous shim , we make the shim to accept no argument
...
class TBWriter(_TBWriter):
    def __init__(self, *args, **kwargs):
        ...
        self.step = 0
        self.writer = self  # engines call writer.add_scalar(...)

    def set_step(self, step=None):
        if step is None:
            self.step += 1          # allow no-arg calls
        else:
            self.step = int(step)
...
def get_args():
    ...
    parser.add_argument('--nb_classes', default=2, type=int, help='Number of classes (for compatibility)')
